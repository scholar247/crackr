...
import {
  INITIAL_HISTORY,
  DEFAULT_QUERY,
} from "@/components/sql_playground/mockData";
...
import { SEED_CONNECTIONS } from "@/components/sql_playground/mockData";
// ─── Main App ─────────────────────────────────────────────────────────────────

export default function App({ params }: { params: Promise<{ db: string }> }) {
  ...
  const [selectedDb, setSelectedDb] = useState<DbConnection>(
    SEED_CONNECTIONS[0],
  );
  useEffect(() => {
    const fetchDb = async () => {
      const { db } = await params;
      const dbConn = SEED_CONNECTIONS.find((conn) => conn.template === db);
      setSelectedDb(dbConn || SEED_CONNECTIONS[0]);
    };
    fetchDb();
  }, []);
  ...
  const updateTabTitle = (sql: string, template: string) => {
    ...
    const title = firstLine.length > 0 ? firstLine : `${template} Query`;
  ...
  const runQuery = async (id: string) => {
    ...
    setTabs((ts) =>
      ts.map((t) => (t.id === id ? { ...t, isRunning: true } : t)),
    );
    ...
    try {
      ...
      setStatusMsg({
        text: data.status ? "Query executed successfully" : data.error,
  ...
  const handleFormat = () =>
    setTabQuery(activeTab, formatSQL(currentTab.query));
            ...
            >
              {(["results", "history"] as BottomPanel[]).map((panel) => (
                <button
                  ...
                  style={{
                    borderColor:
                      bottomPanel === panel ? primaryColor : "transparent",
                    color:
                      bottomPanel === panel
                        ? primaryColor
